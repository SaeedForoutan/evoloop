Relaxed random starts keep landing in mediocre basins — the population's better
programs all begin from structured row layouts instead. Adopt that, and spend
the remaining budget perturbing the incumbent rather than starting over from
noise each time.

```python
"""Structured starts plus incumbent perturbation, replacing random restarts."""
import time
...
def construct_packing(n, random_seed: int):
    ...
    while time.monotonic() < deadline:
        trial = np.clip(best_centers + rng.normal(0.0, strength, size=(n, 2)),
                        0.005, 0.995)
        try:
        ...
        if total > best_sum:
            ...
            strength = max(strength * 0.95, 0.01)
